...
bool symbol_table::insert(symbol_info* symbol) {
    if(current_scope != NULL) return current_scope->insert_in_scope(symbol);
    return false;
}

bool symbol_table::remove(symbol_info* symbol) {
    if(current_scope != NULL) return current_scope->delete_from_scope(symbol);
    return false;
}
...
void symbol_table::print_current_scope(ofstream& outlog) {
    if(current_scope != NULL) current_scope->print_scope_table(outlog);
}
